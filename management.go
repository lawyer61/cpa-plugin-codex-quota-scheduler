...
<div class="quotaList">{{if not .FiveHour.Missing}}<div class="quota-row"><div class="quota-head"><span class="quota-title">{{.FiveHour.Label}}</span><span class="quota-remaining">{{.FiveHour.DisplayText}}</span><span class="quota-reset"><span class="quota-reset-label" data-label="5 小时额度">5 小时额度 重置：</span><span class="localTime quotaResetTime" data-time="{{.FiveHour.ResetText}}">{{.FiveHour.ResetText}}</span></span></div><div class="quota-bar"><div class="quota-fill {{if .FiveHour.Exhausted}}danger{{else if le .FiveHour.RemainingPercent 20.0}}warn{{end}}" style="width:{{printf "%.0f" .FiveHour.RemainingPercent}}%"></div></div></div>{{end}}<div class="quota-row"><div class="quota-head"><span class="quota-title">{{.LongWindow.Label}}</span><span class="quota-remaining">{{.LongWindow.DisplayText}}</span>{{if not .LongWindow.Missing}}<span class="quota-reset"><span class="quota-reset-label" data-label="{{.LongWindow.Label}}">{{.LongWindow.Label}} 重置：</span><span class="localTime quotaResetTime" data-time="{{.LongWindow.ResetText}}">{{.LongWindow.ResetText}}</span></span>{{end}}</div><div class="quota-bar"><div class="quota-fill {{if .LongWindow.Exhausted}}danger{{else if le .LongWindow.RemainingPercent 20.0}}warn{{end}}" style="width:{{printf "%.0f" .LongWindow.RemainingPercent}}%"></div></div></div></div>
